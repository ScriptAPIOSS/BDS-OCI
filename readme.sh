...
echo "  <tr><th><strong>Property Name</strong></th><th><strong>ENV</strong></th><th><strong>Type</strong></th><th><strong>Enums</strong></th><th><strong>Note</strong></th></tr>" >> README.md
...
do
    ...
    note=`cat property-definitions.json | jq -r ".[\"${c}\"].allowed | select (. != null) | . | map(\"<code>\" + . + \"</code><br>\")[] // \"\" "`
    echo -e "<tr><td>${c}</td><td><code>${env_name}</code></td><td>${type}</td><td>${enums}</td><td>${note}</td></tr>\n" >> README.md
done
